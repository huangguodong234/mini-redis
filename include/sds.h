 ...
 * ③ 惰性释放 vs 缓存池（内存池）——一字之差，机制完全不同：
 *    - 惰性释放（sdsclear）：内存留给【同一个 sds 自己】。
 *      这个 sds 下次还要继续 append，直接复用自己身上那块已分配的
 *      alloc 空间。它解决的是一次性的"扩→缩→再扩"抖动。
 *    - 缓存池/内存池（buffer pool）：把【释放掉的块】收进一个池子，
 *      将来【另一个对象】需要时从池里"复用"同一块内存（拿 free 链表
 *      里的块，跳过 malloc）。它解决的是跨对象反复 malloc/free 的
 *      系统调用与碎片开销。这是两套完全不同的复用对象。
 *
 * ④ 为什么 mini-redis 用不上缓存池（buffer pool）？
 *    池化复用的收益建立在"被换手的对象尺寸稳定、命中率高"上。而本
 *    项目的 key / value / member 长度是随机分布的（"OK" vs 几百 KB
 *    都可能有），对象大小不固定：
 *      - 池子里的块很难被"同尺寸"的新对象命中 → 命中率趋近 0，
 *        池子逐渐变成一堆用不上的废块；
 *      - 找不到合适块时还是要 malloc/realloc，等于每次多一次池子查找的白白
 *        开销，净收益为负。
 *    正如 §四 ② 所述，惰性释放只针对"同一个 sds 自己"的复用，范围被
 *    严格限制在长生命周期、反复读写的 sds（如每个连接的 querybuf）。
 *    综合下来，mini-redis 的内存复用只需要惰性释放这一招就够；引入
 *    跨对象的缓存池属于过度设计——底层 glibc malloc 本身已按大小分桶
 *    （size-class）缓存空闲块，再做一层应用层池子收益极小、风险不小。
 *    （参考：真正的 Redis zmalloc 也没有做通用的"响应缓存池"。）
