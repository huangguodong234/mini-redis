...
#define BUFFER_SIZE 4096   //一次读取数据的最大字节数。  加大缓冲区，防止长命令被截断
...
void handls_command(int client_fd,Command *cmd,Storage *store)  //(文件描述符，解释器，存储器-里面有命令执行代码)
{
    //空命令
    if (cmd->argc<1){
...
int main() {
    ...
    if (listen(server_fd, BACKLOG) == -1) {
     ...
     printf("Mini-Redis 服务器运行在 127..0.0.1:%d (按 Ctrl+C 优雅退出)\n", PORT);
    ...
    while((n=read(client_fd,buffer,BUFFER_SIZE-1))>0){
        ...
        if(cmd){
            handls_command(client_fd,cmd,store);
            free_command(cmd);
        }else{
